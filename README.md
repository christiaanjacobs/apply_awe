...
## Data
The ```data``` directory contains speech features extracted from word segments, for example, MFCCs. The features are stored in a numpy archive ```filename.npz```. The file stores a list of sequences with size ```L x T x D``` where ```L``` is the number of word segments, ```T``` is the arbitrary length of a word segment and ```D``` is the dimensionality of a single speech feature.
